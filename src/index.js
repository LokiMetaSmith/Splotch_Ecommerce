...
async function BootStrap() {
    ...
    const designImageUpload = document.getElementById('design-image-upload');
    const cutFileUpload = document.getElementById('cut-file-upload');
    ...
    if (getStartedPrompt) getStartedPrompt.addEventListener('click', showEditor);
    if (previewArea) previewArea.addEventListener('click', showEditor);
    if (cancelEditBtn) cancelEditBtn.addEventListener('click', hideEditor);
    if (okEditBtn) okEditBtn.addEventListener('click', hideEditor);


    if (designImageUpload) {
        designImageUpload.addEventListener('change', handleFileChange);
    }

    if (cutFileUpload) {
        // For now, we assume both uploads can be handled by the same function.
        // This might need to be changed later to handle cutlines specifically.
        cutFileUpload.addEventListener('change', handleFileChange);
    }
