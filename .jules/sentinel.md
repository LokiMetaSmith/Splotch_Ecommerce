...
## 2026-03-01 - WAF Bypass via JSON Stringification Optimization
**Vulnerability:** A "fast path" optimization in the WAF middleware attempted to validate request bodies by checking `JSON.stringify(req.body)` against regex patterns. This was vulnerable because `JSON.stringify` escapes control characters (e.g., newline `\n` becomes `\\n`), causing regex patterns designed for raw input (e.g., `\s+` for whitespace) to fail to match. An attacker could bypass the WAF by injecting a newline into a malicious payload (e.g., `UNION\nSELECT`), which would pass the fast check and skip the thorough recursive validation.
**Learning:** Security optimizations that transform data representation (like object -> JSON string) often introduce subtle bypass vectors if the validation logic (regex) doesn't account for the transformation's side effects (escaping). "Fast paths" are a common source of security regressions because they prioritize performance over correctness/completeness.
**Prevention:** Avoid validating serialized data representations when the goal is to validate the raw data. If performance is a concern, optimize the recursive validation logic or use a dedicated, high-performance validation library. Never skip security checks based on a transformed, potentially misleading representation of the input.

## 2026-03-02 - WAF Recursion Depth DoS
...
## 2026-03-03 - Denial of Service via Image Processing Bomb
**Vulnerability:** The `calculateMaterialUsage` function in `server/utils/materialCalculator.js` processed uploaded images using `Jimp.read()` without first checking the image dimensions. An attacker could upload a "pixel bomb" (a small file that decompresses to massive dimensions, e.g., 50,000x50,000 pixels) or a very high-resolution valid image. This would cause `Jimp` to allocate enormous amounts of memory (hundreds of MBs or GBs) to create the bitmap, potentially crashing the Node.js process (OOM) or freezing the CPU, leading to a Denial of Service.
**Learning:** Image processing libraries often decode compressed formats (like JPEG/PNG) into raw bitmaps in memory. A 10MB JPEG can easily become a 500MB bitmap (width * height * 4 bytes). Relying on file size limits alone is insufficient because compression ratios vary wildly.
**Prevention:** Always parse the image headers to check dimensions *before* fully decoding the pixel data. Use lightweight libraries like `image-size` to peek at dimensions. Enforce a strict pixel count limit (e.g., 50MP) in addition to file size limits to prevent memory exhaustion attacks.
