...
describe("Order API Endpoints", () => {
  ...
  beforeAll(async () => {
    ...
    fs.copyFileSync(
      ...
      path.join(uploadsDir, "design.png"),
    );
  ...
  const getAuthToken = (
    ...
    return jwt.sign({ username, email, role }, privateKey, {
      algorithm: "RS256",
      expiresIn: "1h",
      header: { kid },
    });
  ...
  describe("POST /api/create-order", () => {
    it("should create an order successfully", async () => {
      ...
      expect(db.data.orders[res.body.order.orderId].telegramMessageId).toBe(
        123,
      );
    });
    ...
    it("should support different billing and shipping addresses", async () => {
      ...
      expect(order.billingContact.addressLines).toEqual([
        "200 Wall St, Suite 5",
      ]);
      ...
      const lastPaymentCall =
        mockSquareClient.payments.create.mock.calls[
          mockSquareClient.payments.create.mock.calls.length - 1
        ][0];
      expect(lastPaymentCall.billingAddress.addressLine1).toBe(
        "200 Wall St, Suite 5",
      );
      expect(lastPaymentCall.billingAddress.locality).toBe("New York");
      expect(lastPaymentCall.billingAddress.administrativeDistrictLevel1).toBe(
        "NY",
      );
      expect(lastPaymentCall.billingAddress.postalCode).toBe("10005");

      expect(lastPaymentCall.shippingAddress.addressLine1).toBe(
        "100 Shipping Way",
      );
      expect(lastPaymentCall.shippingAddress.locality).toBe("Oklahoma City");
      expect(lastPaymentCall.shippingAddress.administrativeDistrictLevel1).toBe(
        "OK",
      );
    });
  });
  ...
  describe("POST /api/order/estimate", () => {
    it("should calculate breakdown for local pickup without requiring CSRF token", async () => {
      const res = await request(app).post("/api/order/estimate").send({
        subtotalCents: 1000,
        areaInSqIn: 10,
        destinationState: "OK",
  ...
  describe("GET /api/orders/my-orders", () => {
    it("should return orders for the authenticated user", async () => {
      ...
      db.data.orders[other1] = {
        orderId: other1,
        billingContact: { email: "other@example.com" },
  ...
  describe("GET /api/orders/search", () => {
    it("should allow user to search their orders", async () => {
      ...
      db.data.orders[search123] = {
        orderId: search123,
        billingContact: { email },
        receivedAt: "2023-01-01",
      };
      db.data.orders[search456] = {
        orderId: search456,
        billingContact: { email },
        receivedAt: "2023-01-02",
      };
    ...
    it("should not find other users orders even if ID matches query", async () => {
      ...
      db.data.orders[secret123] = {
        orderId: secret123,
        billingContact: { email: "admin@example.com" },
      };
      await db.write();
      rebuildIndex();
    ...
    it("should find other users orders if user is admin", async () => {
      ...
      db.data.orders[secret123] = {
        orderId: secret123,
        billingContact: { email: "customer@example.com" },
      };
      await db.write();
      rebuildIndex();
  ...
  describe("POST /api/orders/:orderId/status - HOLD_FOR_PICKUP", () => {
    it("should transition order to HOLD_FOR_PICKUP and record holdForPickupAt", async () => {
      ...
      db.data.users["admin_pickup"] = {
        email: adminEmail,
        username: "admin_pickup",
        role: "admin",
      };
      ...
      db.data.orders[orderId] = {
        ...
        deliveryMethod: "pickup",
        billingContact: {
          email: "pickup_customer@example.com",
          givenName: "Jane",
          familyName: "Doe",
        },
  ...
  describe("Non-Volatile Audit Logging & 30-Day Retention Flush", () => {
    ...
    it("should record order transition history and actor when status changes", async () => {
      ...
      const lastEvent =
        orderInDb.statusHistory[orderInDb.statusHistory.length - 1];
        ...
        statusHistory: [
          {
            ...
            fromStatus: null,
            toStatus: "NEW",
            actor: { type: "customer", id: "customer3@example.com" },
          },
        ],
      };
      await db.write();
